...
PhoneHome.prototype.performAction = function(action){
    var self = this;
    ...
    function respondToPhoneHome(){
        self.contactHost(context).catch(logPhoneHomeError).then(function(output){
            ...
            self.aws.optionallyResetS3(aws_keys,function(){
                if (action)
                    self.performHostAction(action,context).then(setCheckTimerCallback).catch(logPhoneHomeError);
                else if (context.state === oldState)
                    self.setCheckTimer();
                else
                    self.considerUploadAction(context,setCheckTimerCallback,logPhoneHomeError);
            });
        });
    }

    (context.state === 'configured' ? self.processPolicies(context) : self.passThroughPromise()).catch(respondToPhoneHome).then(respondToPhoneHome);
};
...
PhoneHome.prototype.performHostAction = function(action,context){
    ...
    return new Promise(function(resolve,reject){
        ...
        self.contactHost(context).then(resolve).catch(reject);
    });
};
...
PhoneHome.prototype.downloadCustomizers = function(context,resolve,reject){
    ...
    function processNextFile(){
        if (contents.length === 0) return self.considerUploadAction(context,resolve,reject);
...
PhoneHome.prototype.considerUploadAction = function(context, resolve, reject){
    var self = this;
    ...
    context.info = self.readLocalInfo();
    self.processPolicies(context).catch(reject).then(function(){
        self.contactHost(context).then(resolve).catch(reject);
    });
};
...
PhoneHome.prototype.upgradeSelf = function(context,resolve,reject){
    ...
    child_process.exec(self.config.settings.upgrade_command,function(error,stdout,stderr) {
        if (error) {
            context.action = context.action ? context.action + '+error' : 'error';
            context.error = error;
        }

        self.contactHost(context).catch(reject).then(function(){
            if (error)
                resolve(null);
            else
                process.exit(0);
    ...
    return new Promise(function(resolve,reject){
        ...
        function processNextPolicy(){
            if (policies.length > 0)
                self.applyPolicy(policies.shift(),processNextPolicy,reject);
            else {
...
PhoneHome.prototype.applyPolicy = function(policy,resolve,reject){
    this.handlerForPolicy(policy).apply(this.context,_.extend(this.config.settings,policy),resolve,reject);
};

PhoneHome.prototype.handlerForPolicy = function(policy){
    var self = this;

    var handlerKey = policy.handler || 'upload';
    if (self.handlerLookup[handlerKey]) return self.handlerLookup[handlerKey];

    var policyPath = __dirname + '/policy-' + handlerKey;
    if (fs.existsSync(policyPath + '.js')) return self.handlerLookup[handlerKey] = new require(policyPath)();

    self.logger.error('handler not found: ' + handlerKey);

    return self.handlerLookup[handlerKey] = {reset: function(){ self.logger.message('no-op reset: ' + handlerKey); },apply: function(context,settings,resolve,reject){ self.logger.message('no-op apply: ' + handlerKey); resolve(null); }};
};
