...
PhoneHome.prototype.handlerForPolicy = function(policy){
    ...
    var policyModule = 'policy-' + handlerKey;
    if (fs.existsSync( __dirname + '/' + policyModule + '.js')){
        var PolicyClass = self.helpers.requireLIB(policyModule);
        self.handlerLookup[handlerKey] = new PolicyClass();
    } else {
        self.logger.error('handler not found: ' + handlerKey);
        self.handlerLookup[handlerKey] = {reset: function(){ self.logger.message('no-op reset: ' + handlerKey); },apply: function(context,settings,resolve,reject){ self.logger.message('no-op apply: ' + handlerKey); resolve(null); }};
    }
