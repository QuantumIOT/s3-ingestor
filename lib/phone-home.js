...
function PhoneHome(emitter,version){
    ...
    var HostHandler = self.helpers.requireLIB(self.config.settings.host_handler);
    self.hostHandler = new HostHandler();
...
PhoneHome.prototype.handlePhoneHomeEvent = function(action){
    ...
    var needsRegistration = self.hostHandler.registrationRequired();
    ...
    function respondToPhoneHome(){
        self.hostHandler.contact(context).then(function(output){
            ...
            self.aws.optionallyResetS3(aws_keys,function(){
                if (self.hostHandler.registrationRequired())
                    self.setCheckTimer();
...
PhoneHome.prototype.performHostAction = function(action,context){
    ...
    return new Promise(function(resolve,reject){
        ...
        self.hostHandler.contact(context).then(resolve,reject);
    });
};
...
PhoneHome.prototype.reapplyPolicies = function(context, resolve, reject){
    ...
    context.info = self.readLocalInfo();
    self.processPolicies(context).then(function(){ self.hostHandler.contact(context).then(resolve,reject); },reject);
};
...
PhoneHome.prototype.upgradeSelf = function(context, resolve, reject){
    ...
    self.helpers.processExec(self.config.settings.upgrade_command,function(error,stdout,stderr) {
        ...
        self.hostHandler.contact(context).then(function(){
            if (error)
                resolve(null);
            else
