...
PhoneHome.prototype.performAction = function(action){
    ...
    (context.state === 'configured' ? self.processPolicies(context) : self.passThroughPromise()).then(respondToPhoneHome,respondToPhoneHome);
};
...
PhoneHome.prototype.rebootSystem = function(context, resolve, reject){
    ...
    child_process.exec(self.config.settings.reboot_command,function(error,stdout,stderr) {
        if (error) {
            context.action = context.action ? context.action + '+error' : 'error';
            context.error = error;
        }
