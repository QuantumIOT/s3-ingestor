...
function PhoneHome(emitter,version){
    ...
    self.contextFile = self.config.home_dir + '/s3-ingestor-context.json';
...
PhoneHome.prototype.downloadCustomizers = function(context,resolve,reject){
    ...
    function processNextFile(){
        ...
        s3.getObject({Bucket: self.config.settings.s3_bucket,Key: entry.Key},function(err,data){
            ...
            var customizersPath = (self.config.home_dir == '.' ? self.helpers.processCWD() : self.config.home_dir) + '/customizers/';
            self.helpers.mkdir(customizersPath,function(err){
                var parts = entry.Key.split('/');
                self.helpers.writeFile(customizersPath + parts[parts.length - 1],data.Body,function(err){
                    if (err) return reportError(err);
