...
function PhoneHome(emitter,version){
    ...
    self.contextFile = self.config.instanceJSON('-context');

    self.aws = new AWS(self.config.copySettings());

    self.host = self.helpers.bestHost(emitter,self.config.settings);
...
PhoneHome.prototype.setCheckTimer = function(){
    ...
    self.host && self.host.setWatchdogTimer();

    self.eventFinished && self.eventFinished();
};

PhoneHome.prototype.readLocalInfo = function(allInfo){
    var hostname = this.config.settings.hostname_override ? this.config.settings.hostname_override : os.hostname();
    return !allInfo ? {hostname: hostname} : {
        hostname:   hostname,
        hosttype:   os.type(),
...
PhoneHome.prototype.applyPolicy = function(context,policy,resolve,reject){
    this.handlerForPolicy(policy).apply(context,this.config.copySettings(policy),this.host,resolve,reject);
};
...
PhoneHome.prototype.handlerForPolicy = function(policy){
    ...
    if (PolicyClass)
        ...
        self.handlerLookup[handlerKey] = {
            ...
            apply: function(context,settings,host,resolve,reject){ self.logger.message('no-op apply: ' + handlerKey); resolve(null); }
        };
    }
