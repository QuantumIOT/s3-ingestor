...
PolicyDownload.prototype.apply = function(context,settings,host,resolve,reject) {
    var self = this;
...
PolicyDownload.prototype.considerOneKey = function(key){
    ...
    s3.headObject({Bucket: self.settings.s3_bucket,Key: key},function(err,result){
        ...
        if (oldStats) {
            if (!lastTimestamp || (oldStats.size === s3FileSizeInBytes && result.LastModified.toString() === lastTimestamp.toString())) {
