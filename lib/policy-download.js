...
PolicyDownload.prototype.validateFileSize = function(local_file, s3_key){
    var localFileSizeInBytes = fs.statSync(local_file)["size"];
    console.log('Local file size in bytes:', localFileSizeInBytes);

    var s3 = self.aws.configureS3();
    s3.headObject({Bucket: self.settings.s3_bucket,Key: s3_key},function(err,result) {
        if (err) return self.recordError(err);

        var s3FileSizeInBytes = parseInt(result.ContentLength);
        console.log('S3 file size in bytes:', s3FileSizeInBytes);

        if (localFileSizeInBytes < s3FileSizeInBytes){
            // here perhaps we should delete this local file and retry to download it
        }
    })
};
