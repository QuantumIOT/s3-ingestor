...
PolicyDownload.prototype.apply = function(context,settings,resolve,reject){
	...
	// The implementation below for downloading files assumes that we get a JSON object like the following from self.settings
	/**
	 { debug: true,
		api_port: 4567,
		host_service: 'http',
		...
		iam_reset_period: 20,
		s3_timeout: 300000,
		s3_bucket: 'qiot-smg-datalake-dev-kbhattarai',
		s3_customizers_prefix: 'code/s3-ingestor/customizers/',
		upgrade_command: 'date',
		policies:
		[ { handler: 'download',
			file_keys: [Object],
			target_directory: '/Users/krishna_qiot/Desktop/magic/' } ],
			orientation: 'portrait-left',
		store_id: '003',
		handler: 'download',
		file_keys:
		[ 'Stores/Colleyville/Display_Right_Landscape_Normal/test1.jpg',
			'Stores/Colleyville/Display_Right_Landscape_Normal/test2.jpg' ],
			target_directory: '/Users/krishna_qiot/Desktop/magic/',
		aws_keys:
		{ access_key_id: 'AKIAJVQLOZHFP7YK6CGA',
			secret_access_key: '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' }
	 }*/
	 ...
	 * It automatically takes care of timestamp checks and is also capable to removing content on both sides to maintain "sync"
