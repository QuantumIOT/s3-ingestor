...
QiotMqttHost.prototype.setWatchdogTimer = function(){
    ...
    if (self.emitter) {
        self.logger.debug('start watchdog timer');

        self.watchdogTimer = setTimeout(function (){self.emitter.emit('phonehome','watchdog');},self.config.settings.heartbeat_period * 2 * 1000);
    }
};
