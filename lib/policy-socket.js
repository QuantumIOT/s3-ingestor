...
PolicySocket.prototype.startUpload = function(){
    ...
    self.popClient = self.redis.createClient();
...
PolicySocket.prototype.checkQueue = function(){
    ...
    self.popClient.brpop(self.settings.socket_queue,self.settings.queue_timeout || 15).catch(self.logErrorCallback).then(function(response){
        ...
        if (!self.s3) {
            ...
            return self.popClient.rpush(self.settings.socket_queue,payload);
        }
...
PolicySocket.prototype.ensureSocket = function(){
    ...
    self.socket.on('data',function(data){
        ...
        if (string == lastString) {
            ...
            self.pushClient.lpush(DEFAULT_SOCKET_QUEUE,payload).catch(self.logErrorCallback).then(function(){
