...
PolicyUpload.prototype.buildkey = function(filename){
    var self = this;
    
    var suffix = self.helpers.trimPrefix(filename,self.settings.input_remove_prefix || '');

    if (typeof(self.settings.customizer) === 'function')
        suffix = self.settings.customizer(suffix,self.settings);
    else if (self.settings.customizer)
        suffix = null;

    return suffix ? (self.settings.output_key_prefix || '') + suffix : null;
};
...
PolicyUpload.prototype.processOneFile = function(filename){
    ...
    fs.stat(filename,function(err,stats){
        if (err) {
            ...
            return _.defer(self.processNextFile);
        }
        var key = undefined;
        var lastSeen = self.lastSeenList[filename];
        if (stats.isDirectory())
            _.defer(self.processNextFile);
        else if (!(key = self.buildkey(filename))) {
...
PolicyUpload.prototype.processNextFile = function(){
    ...
    if (self.currentFiles.length > 0)
        processOneFile(self.currentFiles.shift());
    else
